  ...
  class CertificateReport
    ...
    def check_certificate(file, line, certificate)
      ...
      if certificate.not_after + 3600*24*30*2 < Time.now.utc then
        @certificates << CertificateExpirationInfo.new(file, line, certificate.subject, certificate.not_after)
      end
    end
    ...
    def to_s
      ...
      @certificates.sort! { |a,b| a.not_after <=> b.not_after }

      @expired = @certificates.select { |x| x.not_after <= stop }
      @one_week = @certificates.select { |x| x.not_after > stop && x.not_after <= stop + 3600*24*7 }
      @two_week = @certificates.select { |x| x.not_after > stop + 3600*24*7 && x.not_after <= stop + 3600*24*7*2 }
      @one_month = @certificates.select { |x| x.not_after > stop + 3600*24*7*2 && x.not_after <= stop + 3600*24*30 }
      @two_month = @certificates.select { |x| x.not_after > stop + 3600*24*30 && x.not_after <= stop + 3600*24*30*2 }
      erb = ERB.new <<EOT, nil, '<>'
...
<% @expired.each do |crt| %>
  * <%= crt.file %>:<%= crt.line %>
    subject:   <%= crt.subject %>
    not_after: <%= crt.not_after %> (<%= n = ((Time.now.utc - crt.not_after) / (2600 * 24)).ceil %> day<%= 's' if n != 1 %> ago)
<% end %>
<% end %>
...
<% @one_week.each do |crt| %>
  * <%= crt.file %>:<%= crt.line %>
    subject:   <%= crt.subject %>
    not_after: <%= crt.not_after %> (<%= n = ((crt.not_after - Time.now.utc) / (3600 * 24)).floor %> day<%= 's' if n != 1 %> left)
<% end %>
<% end %>
...
<% @two_month.each do |crt| %>
  * <%= crt.file %>:<%= crt.line %>
    subject:   <%= crt.subject %>
    not_after: <%= crt.not_after %> (<%= ((crt.not_after - Time.now.utc) / (3600 * 24)).floor %> day<%= 's' if n != 1 %> left)
<% end %>
<% end %>
EOT
      erb.result(binding)
